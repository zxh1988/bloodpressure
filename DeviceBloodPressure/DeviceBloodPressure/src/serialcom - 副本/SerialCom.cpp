...
int CSerialCom::ReadFromPort(char **pBuff, IN int nSize, unsigned int timeout)
{
	...
	DWORD dwReadBytes = 0;
	char szReadData[1024] = "\0";
	DWORD dwErrorFlags;
	char *pReadData = szReadData;
	dwReadBytes = min(dwReadBytes, nSize);

	COMSTAT ComStat;
	ClearCommError(m_hcom,&dwErrorFlags,&ComStat);

	if (ReadFile(m_hcom, pReadData, nSize, &dwReadBytes, NULL))
	{
		*pBuff = new char[dwReadBytes + 1];
		*(*pBuff + dwReadBytes) = '\0';
		memcpy(*pBuff, pReadData, dwReadBytes);
		return dwReadBytes;
	}
	else
	{
...
#define READ_TIMEOUT      2000      // milliseconds
	...
	WaitCommEvent(m_hcom, &dwEvtMask, &tOverLaped);
	...
	dwReadBytes = min(dwReadBytes,(DWORD)ComStat.cbInQue);
	if (ReadFile(m_hcom, pReadData, dwReadBytes, &dwReadBytes, NULL))
	{
		*pBuff = new char[dwReadBytes + 1];
		*(*pBuff + dwReadBytes) = '\0';
		memcpy(*pBuff, pReadData, dwReadBytes);
